...
def connect(root):
    ...
    while len(level) > 0:
        ...
        connect_nextRight(nodes)

        del level[current_level]
        current_level += 1

        nodes_in_same_level = []
        for node in nodes:
            if node.left:
                nodes_in_same_level.append(node.left)
            if node.right:
                nodes_in_same_level.append(node.right)

        if len(nodes_in_same_level) > 0:
            level[current_level] = nodes_in_same_level


def connect_nextRight(nodes):
